                                    ...
                                    struct vregion *vregion)
{
    ...
    err = memobj_create_one_frame_one_map(memobj, size, 0);
    if (err_is_fail(err)) {
        return err_push(err, LIB_ERR_MEMOBJ_CREATE_ONE_FRAME);
    }
    ...
    err = vregion_map_fixed(vregion, get_current_vspace(), &memobj->m, 0, size, addr, flags);
    if (err_is_fail(err)) {
        return err_push(err, LIB_ERR_VREGION_MAP);
    }
                                    ...
                                    struct vregion **retvregion)
{
    ...
    struct memobj_one_frame_one_map *memobj   = NULL;
    struct vregion *vregion = NULL;

    size = ROUND_UP(size, BASE_PAGE_SIZE);

    // Allocate space
    memobj = calloc(1, sizeof(struct memobj_one_frame_one_map));
    if (!memobj) {
    ...
    err = vspace_map_one_frame_one_map_fixed_attr_no_malloc(addr, size, frame, flags, memobj, vregion);
    if (err_is_fail(err)) {
        free(memobj);
        free(vregion);
        return err;
